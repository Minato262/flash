...
public final class BeanContainer {
     ...
     * 概况
     *
     * 在 Java 的框架体系中, JavaBean 被看成是所有应用的的基石, 主要原因在于她的灵活
     * 轻便，以及能够将变化点封装成代码块，从而简化开发的特点。但用 JavaBean 作为基本
     * 的组件模型。却是来自于常年编程经验的积累，她最初是用于代替的 EJB 的。
     *
     * EJB 是 SUN 制定的一种组件模型，其作用是将可以重复使用的代码打包，从而达到简化开
     * 发的作用。在早期的 JDK 中，他是作为 JavaEE 标准的一部分，写入 JDK 的。事实上在
     * JDK6 的 javax.ejb 中仍然有其最新的改进版，她为 PC 控件提供支持。但在 servlet，
     * jsp 访问，JDBC，applet 等等方面 EJB 被评价为过于笨重。主要是因为 EJB 是为企业
     * 级应用而设计的，企业级应用往往会伴随分布式，事务，高并发等等场景，以至于对于中小型
     * 应用或者其他一些场景而言，EJB 就有些设计过度了。
     *
     ...
     * 优势。而她本身也成为 JavaEE 的一个标准。虽然早就在 JDK 的 java.beans 已经有了许
     * 多相关支持了。
     *
     * 但简单的 JavaBean 直接构成应用，也是有问题的。因为从用户的角度去看 JavaBean，他
     * 与可重用应用程序的代码块，共同构建了组件，而一个组件和一个开关就可以构成一个小程序。
     * 但开发中，你必须分别创建对象，并且它们需要一起使用或者在不同的应用程序或情况下，不同
     * 的组件产生不同的组合来使用。
     *
